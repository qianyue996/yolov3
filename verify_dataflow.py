...
def step(label: str, *values: tuple) -> None:
    ...
    for name, obj in values:
        if isinstance(obj, torch.Tensor):
            print(f"  {name}: shape={tuple(obj.shape)}, dtype={obj.dtype}, "
            ...
            for i, item in enumerate(obj):
                if isinstance(item, torch.Tensor):
                    print(f"    [{i}]: shape={tuple(item.shape)}, "
...
def main() -> None:
    ...
    step("step1: Dataset.__getitem__ 原始输出",
         ("image", raw_img),
    ...
    step("step3: batch_y (即 YOLOLOSS 的 all_targets)",
         ("batch_y", batch_y))
    print("  此时 all_targets 格式: xyxy 归一化到 [0,1]")

    # 4. 模型前向
    print("\n▶ 模型前向，查看 RawPredicts 形状")
    model = YoloBody(anchors=anchors, anchors_mask=anchors_mask,
                     class_names=class_names).to(DEVICE)
    batch_x = images.to(DEVICE)
    outputs = model(batch_x)
    step("step4: 模型输出 RawPredicts",
