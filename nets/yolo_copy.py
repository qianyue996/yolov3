...
class LastLayer(nn.Module):
    ...
    def _createGrid(self, shape):
        ...
        self.shape = (h, w)
        self.stride = 416 / h
        self.anchorScaled = torch.tensor(self.anchor / self.stride, device='cpu')
        grid = torch.ones((b,len(self.anchor),h,w,4),device='cpu')
        gridY, gridX = torch.meshgrid(torch.arange(h), torch.arange(w), indexing="ij")
        grid[...,0] *= gridX.to('cpu').unsqueeze(0)
        grid[...,1] *= gridY.to('cpu').unsqueeze(0)
...
class YOLOv3(nn.Module):
    def __init__(self) -> None:
        ...
        # 得到 75*13*13
        self.predict1 = LastLayer(512, 3 * (5 + 80), anchor=[[116,90],  [156,198],  [373,326]])
        # 得到 75*26*26
        self.predict2 = LastLayer(256, 3 * (5 + 80), anchor=[[30,61],  [62,45],  [59,119]])
        # 得到 75*52*52
        self.predict3= LastLayer(128, 3 * (5 + 80), anchor=[[10,13],  [16,30],  [33,23]])
    ...
    def getWeight(self,yolo):
        ...
        weightData = torch.load("models/yolo_weights.pth", map_location="cpu")
