...
model = YoloBody(anchors_mask=config.anchors_mask, num_classes=80, pretrained=True).to(config.device)
model.load_state_dict(torch.load("checkpoint.pth", map_location=config.device)['model'])
model.eval()
def draw(img, boxes, scores, labels):
    for i in range(len(boxes)):
        x1, y1, x2, y2 = boxes[i]
        cv.rectangle(img, (int(x1), int(y1)), (int(x2), int(y2)), (0, 0, 255), thickness=1)
        cv.putText(img, f'{scores[i]:.2f} {config.class_name[labels[i]]}', (int(x1), int(y1)-5), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), thickness=1)
...
def process(img, input):
    ...
    for i in range(len(output)):
        S = config.feature_map[i]
        stride = config.sample_ratio[i]

        prediction = output[i].squeeze().view(3, 85, S, S).permute(0, 2, 3, 1)
        anchors = torch.tensor(config.anchors, device=config.device)

        boxes, scores, labels = buildBox(i, S, stride, prediction, anchors, config.anchors_mask)
...
def transport(img, to_tensor=True):
    if to_tensor:
        img = cv.resize(img, (config.imgsize, config.imgsize))
        input = cv.cvtColor(img, cv.COLOR_BGR2RGB)
        input = np.transpose(np.array(input / 255.0, dtype=np.float32), (2, 0, 1))
        input = torch.tensor(input).unsqueeze(0).to(torch.float32).to(config.device)
    return img, input
