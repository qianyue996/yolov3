...
class CocoFlatImageDataset(Dataset):
    def __init__(self, img_folder_path, transform=None):
        ...
        for ext in ['jpg', 'jpeg', 'png', 'bmp', 'tif', 'tiff']:
            self.image_paths.extend(glob.glob(os.path.join(img_folder_path, f'*.{ext}')))
        
        if not self.image_paths:
            raise FileNotFoundError(f"No image files found in {img_folder_path}. Please check the path and image extensions.")
...
def calculate_dataset_mean_std(dataset_root, resize_dim=(416, 416), batch_size=64, num_workers=4):
    """
    ...
    dataset = CocoFlatImageDataset(img_folder_path=dataset_root, transform=transform_for_stats)
