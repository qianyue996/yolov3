    ...
    for key, value in config.items():
        if isinstance(value, (int, float)):  # 判断是否是数字类型
            input_components.append(
                gr.Slider(minimum=0.000001, maximum=10, step=0.00001, label=key, value=value)
            )
...
def launch_gradio():
    ...
    with gr.Blocks() as demo:
        ...
        update_btn.click(gradio_interface_update, inputs=input_components, outputs=output_json)  # 点击 "Update Config" 按钮，更新配置
